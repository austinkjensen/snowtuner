...
if TYPE_CHECKING:
    # Avoid an import cycle: snowtuner.experiments.config_delta imports
    # snowtuner.recommenders.sizes, which loads snowtuner.recommenders/__init__,
    # which would re-enter this module before experiments.model is ready.
    # The forward reference resolves correctly for tooling because of the
...
class Recommender(ABC):
    ...
    def propose_experiments(
        ...
        """Optionally propose experiments instead of (or in addition to) advisory
        recommendations.

        v0.2 default: return ``[]``.  A recommender that wants to gather data
        before committing to a recommendation overrides this to return one or
        more ``ProposedExperiment``s — typically built via
        ``snowtuner.experiments.recipes.PRESET_RECIPES``.

        The orchestrator inserts these into ``app.experiments`` with status
        ``PROPOSED`` and surfaces them in the UI for human acceptance.  The
        engine runs them on demand; the resulting report is what eventually
        becomes a confident recommendation.
        """
        return []
