...
- **Ingestion** - pulls `QUERY_HISTORY`, `WAREHOUSE_METERING_HISTORY`, `WAREHOUSE_EVENTS_HISTORY`, and `SHOW WAREHOUSES` into a local DuckDB.
- **AUTO_SUSPEND tuning** - cost-minimizing survival analysis on the idle gaps between busy periods, derived from `QUERY_HISTORY`.
- **Warehouse right-sizing** - four transparent rules over spill, queueing, and latency, targeting `WAREHOUSE_SIZE`.
- **Multi-cluster reduction** - lowers `MIN_CLUSTER_COUNT` / `MAX_CLUSTER_COUNT` to the peak Snowflake actually provisioned, plus a margin.
- **Autonomous apply and rollback** - per `(action, warehouse, knob)` opt-in with a confidence threshold, cooldown, and a rollback-budget circuit breaker; every applied change carries a one-click revert.
- **Replay-experiments framework** - in-vitro A/B and benchmark testing of warehouse configs against sampled production queries, with paired t-tests, Bonferroni correction, and confidence intervals. Gen2, QAS, and size changes run through it as preset recipes.
- **Queries explorer** - filter and drill into ingested query history, with a query-family rollup.
- **Interfaces** - a FastAPI HTTP API, a React web UI, and an Admin MCP server (43 tools) for driving it from Claude.
- **Setup and deploy** - an RSA key-pair service user generated by `bootstrap-sql`, plus one-command AWS deployment (CloudFormation, reached over SSM, no public URL).
- **Demo mode** - provisions cooked warehouses that trigger every recommender, so you can watch the whole pipeline on your own account before pointing it at production.

**In preview:** saved query groups (static and dynamic) work through the API and MCP server, but don't have a web-UI surface yet.

Everything else is on the [roadmap](#roadmap); multi-account fleet management, SLO-backed savings, and managed hosting are the planned paid tier.
...
**v0.2 (in flight):** the replay-experiments framework (tuning and benchmark comparisons) and the Queries explorer are shipped; saved query groups are in preview, usable through the API and MCP server with no web UI yet. The next slices:

- **A web UI for saved query groups**, to graduate them out of preview.
